...
const log = (msg) => {
    ...
    console.log(`[MEM-CTRL] [${timestamp}] ${msg}`);
};
...
const getMemberStats = async (req, res) => {
    try {
        ...
        const matchStage = {};
        const isAdmin = (role === 'admin' || role === 'it_sector');
        ...
        log(`Error in getMemberStats: ${error.message}`);
...
const getMembers = async (req, res) => {
    try {
        ...
        const log = (msg) => {
            const now = Date.now();
            console.log(`[getMembers] [+${now - startTime}ms] ${msg}`);
        };

        log('Starting Optimized Flow');
        ...
        let membersQuery = Member.find(dbMatch).select('-transactions -registrationData -signatureImage -profileImage -documentPdf').lean().sort({ joinedDate: -1 });
        const members = await membersQuery;
        log(`Fetched ${members.length} Members`);
        if (members.length > 0) {
            log(`First Member: ${members[0].name}, Has registrationData: ${!!members[0].registrationData}, Keys: ${Object.keys(members[0].registrationData || {}).length}`);
        }
        ...
        log(`Aggregated Transactions for ${members.length} members. Got ${txStats.length} stats rows.`);
        ...
        let data = members.map(m => {
            ...
            return {
                ...
                registrationData: {
                    DEBUG_TEST: "SERVER_VALUE_OK",
                    resident_dob: "1900-01-01"
                }
            };
        });
...
const getMember = async (req, res) => {
    try {
        ...
        log(`Error fetching single member: ${error.message}`);
        res.status(500).json({ success: false, message: 'Server error' });
    }
};
